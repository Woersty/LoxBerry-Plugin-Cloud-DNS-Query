...
# Variables
my $maintemplatefilename 		= "Cloud_DNS_Query.html";
my $errortemplatefilename 		= "error.html";
my $helptemplatefilename		= "help.html";
my $languagefile 				= "language.ini";
my $logfilename 				= "Cloud_DNS_Query.log";
my $template_title;
my $no_error_template_message	= "<b>Cloud_DNS_Query:</b> The error template is not readable. We must abort here. Please try to reinstall the plugin.";
my $version 					= LoxBerry::System::pluginversion();
my $helpurl 					= "http://www.loxwiki.eu/display/LOXBERRY/Cloud_DNS_Query";
my $log 						= LoxBerry::Log->new ( name => 'Cloud_DNS_Query', filename => $lbplogdir ."/". $logfilename, append => 1 );
my $error_message				= "";
...
sub defaultpage 
{
	...
	LoxBerry::Web::lbfooter();
	LOGDEB "Leaving Cloud_DNS_Query Plugin normally";
	LOGEND if $plugin->{PLUGINDB_LOGLEVEL} eq 7;
	exit;
}
...
sub error 
{
	...
	LoxBerry::Web::lbfooter();
	LOGDEB "Leaving Cloud_DNS_Query Plugin with an error";
	LOGEND if $plugin->{PLUGINDB_LOGLEVEL} eq 7;
	exit;
}
